...
flowchart LR
  ...
  Graph --> BMC["Bounded Model Checker<br/>timeline satisfiability"]
...
| Formal timeline verification | The bounded model checker stores `CONTRADICTION` records when Prefetch, Amcache, USN, and MFT cannot satisfy causal time-density constraints. |
...
10. Prove timeline contradictions with bounded model checking.
...
15. Seal the evidence graph with a Merkle-DAG root.
16. Upgrade only if independent artifacts agree.
17. Apply negative controls.
18. Generate report, trace index, and benchmark outputs.
```
